...
def write_imputed_matrices(cpg_density, matrix_list, bin_list):
    ...
    output_imputed_file_pickle = "./%s/PReLIM.Sample.%s.%s_IMPUTED_CPG%s.pickle" % (
        output_directory, input_bam.split("/")[-1], individual_chrom, cpg_density)
    #np.save(output_imputed_file_npy, dict(zip(bin_list, matrix_list)))
    with open(output_imputed_file_pickle, 'wb') as f:
        pickle.dump(dict(zip(bin_list, matrix_list)), f)
    


def write_initial_matrices(output_file, initial_matrices, output_file_pickle):
...
def get_PReLIM_imputed_matrices(bin_size, input_bam, output_directory, individual_chrom):
    ...
    output_file_pickle = "./%s/PReLIM.Sample.%s.%s.pickle" % (output_directory, input_bam.split("/")[-1], individual_chrom)
    write_initial_matrices(output_file, final_results, output_file_pickle)
    ...
    cpgs.append(cpg2_cpgs)
    cpgs.append(cpg3_cpgs)
    cpgs.append(cpg4_cpgs)
    cpgs.append(cpg5_cpgs)
    ...
    for i in range(2, 6):
        ...
        model.train(matrices[i-2], model_file="model_%s_cpg%s" % (i, input_bam.split("/")[-1]))
        ...
        imputed_matrix_list = []
        for j in range(len(imputed_bins)):
            imputed_matrix = postprocess_predictions(imputed_bins[j])
            imputed_matrix_list.append(pd.DataFrame(data=imputed_matrix, columns=cpgs[i-2][j]))
